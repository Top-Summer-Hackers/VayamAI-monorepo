...
contract VayamAI is Ownable, ERC721 {
    ...
    function createDeal(
        ...
        address _invoice = ISmartInvoiceFactory(smartInvoiceFactory).create(
            msg.sender,
            _amounts,
            data,
            ESCROW_TYPE
    ...
    function closeDeal(
        ...
        if (invoice.client == msg.sender && !invoice.isClosedByClient) {
            ...
            invoice.freelancer == msg.sender && !invoice.isClosedByFreelancer
        ) {
    ...
    function _checkInvoiceCompletion(address _invoice) internal {
        bool hasFinised = ISmartInvoiceEscrow(_invoice).terminationTime() <
            block.timestamp;
        uint256[] memory amounts = ISmartInvoiceEscrow(_invoice).getAmounts();
    ...
    function _checkAirdropNFT(address user) internal {
        if (
            balanceOf(user) == 0 &&
            closedInvoicesCountMaxScore[user] >=
            MIN_NUMBER_OF_CLOSED_INVOICES_WITH_5_5
        ) {
