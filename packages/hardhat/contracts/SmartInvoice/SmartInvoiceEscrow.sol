...
contract MockSmartInvoiceEscrow is ISmartInvoiceEscrow, IArbitrable, Initializable, Context, ReentrancyGuard {
   ...
   */
  function init(address _recipient, uint256[] calldata _amounts, bytes calldata _data) external override initializer {
  ...
  function _handleData(bytes calldata _data) internal {
    (
      ...
      address _factory
    ) = abi.decode(_data, (address, uint8, address, address, uint256, bytes32, address, bool, address));
    ...
    require(_terminationTime <= block.timestamp + MAX_TERMINATION_TIME, "duration too long");
   ...
   * @param _details Additional details for the milestones
   */
  function _addMilestones(uint256[] calldata _milestones, bytes32 _details) internal {
  ...
  function lock(bytes32 _details) external payable override nonReentrant {
    ...
    if (resolverType == ADR.ARBITRATOR) {
      disputeId = IArbitrator(resolver).createDispute{value: msg.value}(NUM_RULING_OPTIONS, abi.encodePacked(details));
    }
    locked = true;
   ...
   * @param _details Details of the dispute
   */
  function resolve(uint256 _clientAward, uint256 _providerAward, bytes32 _details) external override nonReentrant {
    ...
    require(_clientAward + _providerAward == balance - resolutionFee, "resolution != remainder");
